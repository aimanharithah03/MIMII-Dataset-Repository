...
Three evaluation modes (set CONFIG["EVAL_MODE"]):
  ...
  "pooled"                 - files from all selected machine IDs are
                              ...
                              train/val/test. Tests "can it detect faults
                              on machines it has already seen."

  "leave_one_group_out"    - trains on every machine ID EXCEPT
                              CONFIG["HOLDOUT_MACHINE_ID"], tests only on
                              that held-out machine. Tests generalization
                              to one new, unseen unit.

  "leave_one_group_out_all"- runs leave_one_group_out once PER machine ID
                              (each takes a turn being the held-out one),
                              then prints + saves a summary table/chart/CSV
                              comparing all of them. This is how you check
                              whether generalization is consistent across
                              machines, or whether some are much easier/
                              harder than others.
...
Per-machine baseline normalization (CONFIG["NORMALIZE_PER_MACHINE"]):
  ...
  unit's own baseline, instead of an absolute baseline learned from other
  physical units.

Decision-threshold tuning (CONFIG["THRESHOLD_MODE"]):

  "auto_f1" (default) - sweeps thresholds from 0.05 to 0.95 on the
                         VALIDATION set only (never the test machine, to
                         avoid leaking test information), and picks the
                         threshold that maximizes F1 for the abnormal
                         class. A precision/recall/F1-vs-threshold plot
                         is saved per run so you can see the trade-off.

  "fixed"             - uses CONFIG["FIXED_THRESHOLD"] as-is (0.5
                         reproduces the original .predict() behavior).
...
Dependencies (install once):
    pip install librosa scikit-learn matplotlib joblib
"""
...
CONFIG = {
    ...
    "MODEL_OUT": r"D:\MIMII Dataset_Big\6_dB_fan\fan_classifier.joblib",
...
def build_dataset(filepaths, labels, sr, n_mfcc):
    ...
    into an (X, y) pair ready for scikit-learn."""
    X, y = [], []
...
def normalize_by_own_normal_baseline(X, y):
    ...
    Meant to be called separately per machine, so each machine's samples
    get compared against that machine's own normal baseline instead of
    an absolute, cross-machine scale."""
    normal_mask = (y == 0)
...
# ============================================================
def tune_threshold_on_validation(clf, X_val, y_val, thresholds):
    ...
    the test set — that's the whole point of tuning here instead of
    on the held-out machine."""
...
def train_evaluate_and_save(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    print("\nStep 4: Training the classifier (RandomForest)...")
    ...
    default_val_pred = clf.predict(X_val)
    ...
    if cfg["THRESHOLD_MODE"] == "auto_f1":
        best, sweep = tune_threshold_on_validation(clf, X_val, y_val, cfg["THRESHOLD_SWEEP"])
    ...
    print("\nStep 8: Saving the trained model + chosen threshold together...")
    joblib.dump({"model": clf, "threshold": chosen_threshold}, model_out)
...
def run_pooled(cfg):
    ...
    train_evaluate_and_save(
        ...
        cm_out=cfg["CONFUSION_MATRIX_OUT"],
        cm_title="MIMII 6_dB_fan — Confusion Matrix (Test Set)",
        threshold_curve_out=cfg["CONFUSION_MATRIX_OUT"].replace(".png", "_threshold_curve.png"),
    )
...
def run_single_leave_one_out(cfg, holdout_id, all_ids):
    ...
    model_out = cfg["MODEL_OUT"].replace(".joblib", f"_loo_{holdout_id}.joblib")
    cm_out = cfg["CONFUSION_MATRIX_OUT"].replace(".png", f"_loo_{holdout_id}.png")
    cm_title = f"MIMII 6_dB_fan — Leave-One-Out Confusion Matrix (test={holdout_id})"
    threshold_curve_out = cm_out.replace(".png", "_threshold_curve.png")
...
def summarize_leave_one_out_results(results, cfg):
    ...
    # Save a CSV next to the model output for easy reference later
    ...
    # Bar chart comparing all machines side by side
